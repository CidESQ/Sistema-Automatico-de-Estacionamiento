...
// Obtener el mes seleccionado de la URL
$mes = $_GET['month'];
$monthName = $_GET['monthName'];
$GLOBALS['monthName'] = $monthName;

$monthName2 = $_GET['monthName'];
$monthName_uppercase = strtoupper($monthName2);
$GLOBALS['monthName2'] = $monthName_uppercase;
...
$query = "SELECT * FROM carro WHERE MONTH(fecha_salida)='$mes' AND YEAR(fecha_salida)='$año'";
$resultado = mysqli_query($conexion, $query);

//Realizar la consulta para la suma
$query2 = "SELECT SUM(total) FROM carro WHERE MONTH(fecha_salida)='$mes' AND YEAR(fecha_salida)='$año'";
...
class PDF extends FPDF
{
    ...
    function Header()
    {
        if ($this->isFirstPage) { // Mostrar el título solo en la primera página
            ...
            $this->Cell(0, 10, 'REPORTE DE LAS GANANCIAS GENERADAS EN EL MES', 0, 1, 'C');
            $this->Cell(0, 10, ' DE '.$GLOBALS['monthName2'] . ' DE 2023', 0, 1, 'C');
            $this->Ln(10);
...
$pdf->Cell(0, 10, 'Los datos que se encuentran en la tabla corresponden a los coches ingresados durante el mes:'.$monthName, 0, 1); 
...
$pdf->Cell(0, 10, 'Las ganancias calculadas corresponden al mes de ' .$monthName , 0, 1); 
